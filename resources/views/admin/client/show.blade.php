        ...
        <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
            <div class="d-flex align-items-center flex-wrap mr-1">
                <div class="d-flex align-items-baseline mr-5">
                    <a class="text-dark" href="{{ route('admin.client.index') }}"><h5 class="text-dark font-weight-bold my-2 mr-5">Clientes</h5></a>
                                        ...
                                        <div class="d-flex justify-content-end">
                                            <div class="dropdown dropdown-inline" data-toggle="tooltip"  data-placement="left">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                                                    @can('cotizaciones')
                                                        <a class="dropdown-item" href="{{ route('admin.quotation.create', ['client' => $client]) }}"><i class="fa fa-sticky-note mr-2"></i> Adjuntar cotización</a>
                                                    @endcan
                                                    @can('facturas')
                                                        <a class="dropdown-item" href="{{ route('admin.invoice.create', ['client' => $client]) }}"><i class="fas fa-file-pdf mr-2"></i> Adjuntar factura</a>
                                                    @endcan
                                                    @can('pagos')
                                                        <a class="dropdown-item" href="{{ route('admin.payment.create', ['client' => $client->id]) }}"><i class="fa fa-credit-card mr-2"></i> Generar un pago</a>
                                                    @endcan
                                                    @can('gastos')
                                                        <a class="dropdown-item" href="{{ route('admin.expense.create', ['client' => $client->id]) }}"><i class="fa fa-calculator mr-2"></i> Generar un gasto</a>
                                                    @endcan
                                                    <a class="dropdown-item" href="{{ route('admin.client.edit', $client) }}"><i class="fa fa-pen mr-2"></i> Editar</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="row">
                    <div class="col-xl-4">
                        ...
                        <div class="card bgi-no-repeat card-stretch gutter-b" style="background-position: right top; background-size: 30% auto; background-image: url({{ asset('assets') }}/media/svg/shapes/abstract-4.svg)">
                            <div class="card-body">
                                @if ($client->user)
                                    <a href="{{ route('admin.user.show', $client->user) }}" class="card-title font-weight-bold text-muted text-hover-primary font-size-h5">Pertenece a</a>
                                    <div class="">
                            ...
                            <div class="card-body px-0">
                                <div class="tab-content ">
                                    ...
                                    <div class="tab-pane active" id="comment" role="tabpanel">
                                        @livewire('admin.client.comment', ['client' => $client])
                                    </div>

                                    <div class="tab-pane" id="payment" role="tabpanel">
                                        @livewire('admin.payment.index', ['client' => $client])
                                    </div>
                                    
                                    <div class="tab-pane" id="expense" role="tabpanel">
                                        @livewire('admin.expense.index', ['client' => $client])
                                    </div>
                                    
                                    <div class="tab-pane" id="service" role="tabpanel">
                                        @livewire('admin.service.index', ['client' => $client, 'type' => 'Mensual'], key('service'))
                                    </div>

                                    <div class="tab-pane" id="project" role="tabpanel">
                                        @livewire('admin.service.index', ['client' => $client, 'type' => 'Proyecto'], key('project'))
                                    </div>
                                    
                                    <div class="tab-pane" id="invoice" role="tabpanel">
                                        @livewire('admin.invoice.index', ['client' => $client], key($client->id))
                                    </div>
                                    
                                    <div class="tab-pane" id="quotation" role="tabpanel">
                                        @livewire('admin.quotation.index', ['client' => $client], key($client->id))
                                    </div>
                                </div>
                            </div>
                            <!--end::Body-->
